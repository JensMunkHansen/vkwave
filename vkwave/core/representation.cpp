...
// Please keep the functions in here in alphabetical order (sort by function names, then by
// parameter types)
...
template <>
std::string_view as_string(const vk::CompositeAlphaFlagBitsKHR flag)
{
  return as_string(static_cast<VkCompositeAlphaFlagBitsKHR>(flag));
}
...
template <>
std::string_view as_string(const vk::SurfaceFormatKHR surface_format)
{
  return as_string(static_cast<VkFormat>(surface_format.format));
}
...
{
  constexpr std::array<std::string_view, sizeof(vk::PhysicalDeviceFeatures) / sizeof(VkBool32)>
    FEATURE_DESCRIPTIONS{ // robustBufferAccess
      "accesses to buffers which are bounds-checked against the range of the buffer descriptor",
      // fullDrawIndexUint32
      "full 32-bit range of indices for indexed draw calls when using a VkIndexType of "
      "VK_INDEX_TYPE_UINT32",
      // imageCubeArray
      "creation of image views of type VK_IMAGE_VIEW_TYPE_CUBE_ARRAY",
      // independentBlend
      "VkPipelineColorBlendAttachmentState settings which are controlled independently "
      "per-attachment",
      // geometryShader
      "geometry shaders",
      // tessellationShader
      ...
      // alphaToOne
      "replacing the alpha value of the fragment shader color output in the multisample coverage "
      "fragment operation",
      // multiViewport
      ...
      "storage buffers and images which support atomic operations in vertex, tessellation, and "
      "geometry shaders",
      // fragmentStoresAndAtomics
      "storage buffers and images which support atomic operations in fragment shaders",
      // shaderTessellationAndGeometryPointSize
      "PointSize built-in decoration being available in the tessellation control, tessellation "
      "evaluation, and "
      "geometry shaders",
      ...
      "arrays of uniform buffers which can be indexed by dynamically uniform integer expressions "
      "in shader code",
      // shaderSampledImageArrayDynamicIndexing
      "arrays of samplers or sampled images which can be indexed by dynamically uniform integer "
      "expressions in "
      "shader code",
      // shaderStorageBufferArrayDynamicIndexing
      "arrays of storage buffers which can be indexed by dynamically uniform integer expressions "
      "in shader code",
      // shaderStorageImageArrayDynamicIndexing
      "arrays of storage images which can be indexed by dynamically uniform integer expressions in "
      "shader code",
      // shaderClipDistance
      ...
      // sparseBinding
      "resource memory which can be managed at opaque sparse block level instead of at the object "
      "level",
...
std::string_view result_to_description(const VkResult result)
{
  switch (result)
  {
    ...
    case VK_ERROR_FRAGMENTED_POOL:
      return "A pool allocation has failed due to fragmentation of the pool's memory. This must "
             "only be returned if "
             "no attempt to allocate host or device memory was made to accommodate the new "
             "allocation. This should "
             "be returned in preference to VK_ERROR_OUT_OF_POOL_MEMORY, but only if the "
    ...
    case VK_ERROR_OUT_OF_DATE_KHR:
      return "A surface has changed in such a way that it is no longer compatible with the "
             "swapchain, and further "
             "presentation requests using the swapchain will fail. Applications must query the new "
             "surface "
             "properties and recreate their swapchain if they wish to continue presenting to the "
             "surface";
    case VK_ERROR_INCOMPATIBLE_DISPLAY_KHR:
      return "The display used by a swapchain does not use the same presentable image layout, or "
             "is incompatible in "
             "a way that prevents sharing an image";
    case VK_ERROR_INVALID_SHADER_NV:
      return "One or more shaders failed to compile or link. More details are reported back to the "
             "application via "
             "VK_EXT_debug_report if enabled";
    case VK_ERROR_OUT_OF_POOL_MEMORY:
      return "A pool memory allocation has failed. This must only be returned if no attempt to "
             "allocate host or "
             "device memory was made to accommodate the new allocation. If the failure was "
             "definitely due to "
    ...
    case VK_ERROR_UNKNOWN:
      return "An unknown error has occurred; either the application has provided invalid input, or "
             "an implementation "
...
std::string_view result_to_description(const vk::Result result)
{
  switch (result)
  {
    ...
    case vk::Result::eErrorInvalidDeviceAddressEXT:
      return "A buffer creation failed because the requested address is not available";
    case static_cast<vk::Result>(VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT):
      return "An operation on a swapchain created with "
             "VK_FULL_SCREEN_EXCLUSIVE_APPLICATION_CONTROLLED_EXT "
             "failed as it did not have exclusive full-screen access. This may occur due to "
             "implementation-dependent "
...
template <>
std::vector<std::string> as_description(vk::SurfaceTransformFlagsKHR bits)
{
...
template <>
std::vector<std::string> as_description(vk::SurfaceTransformFlagBitsKHR bits)
{
  return as_description<vk::SurfaceTransformFlagsKHR>(bits);
}
...
template <>
std::vector<std::string> as_description(vk::CompositeAlphaFlagsKHR bits)
{
  std::vector<std::string> result;
...
*/
template <>
std::vector<std::string> as_description(vk::ImageUsageFlags bits)
{
  std::vector<std::string> result;
